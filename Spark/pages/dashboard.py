...
class DashboardPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(text_frame, text=localization.get("app_title_1", "Картотека"), font=("Inter", 24, "bold"),
                     text_color="#FFFFFF").pack(anchor="w")
        ctk.CTkLabel(text_frame, text=localization.get("app_title_2", "библиотекаря"), font=("Inter", 14),
                     text_color="#E6C619").pack(anchor="w", pady=(0, 0))

        # --- НАВИГАЦИЯ ---
        self.nav_zone = ctk.CTkFrame(self.sidebar_inner, fg_color="transparent")
        self.nav_zone.pack(side="top", fill="x")

        self.create_nav_btn(localization.get("main"), is_active=True,
        ...
        # УМНЫЙ ПОИСК АВАТАРКИ (учитывает регистр букв в Linux)
        ...
        ctk.CTkLabel(profile_text_frame, text="СЕЗИМАЙ", text_color="white", font=("Inter", 14, "bold")).pack(
            anchor="w")
        ctk.CTkLabel(profile_text_frame, text=localization.get('librarian'), text_color="#BEAC64",
                     font=("Inter", 14)).pack(anchor="w")
        ...
        self.search_bar = ctk.CTkEntry(self.header_frame, placeholder_text=localization.get("search_placeholder"),
                                       width=510, height=42, corner_radius=14, fg_color="#C4C4C4", text_color="black",
                                       border_width=0, font=("Inter", 14))
        self.search_bar.pack(side="left")

        self.add_book_btn = ctk.CTkButton(self.header_frame, text=localization.get("add_book"), fg_color="#BEAC64",
                                          text_color="black", width=193, height=42, command=self.show_add_book_modal,
                                          corner_radius=14, font=("Inter", 14, "bold"))
        ...
        self.lbl_books = self.create_stat_card(self.stats_frame, localization.get("books_on_loan"), str(stats[0]),
                                               "check.png", col=0)
        self.lbl_overdue = self.create_stat_card(self.stats_frame, localization.get("overdue"), str(stats[1]),
                                                 "alert.png", col=1)
        self.lbl_readers = self.create_stat_card(self.stats_frame, localization.get("new_readers"), str(stats[2]),
                                                 "user_icon.png", col=2)
        ...
        # Graph Box
        self.graph_box = ctk.CTkFrame(self.middle_frame, fg_color="#D9D9D9", corner_radius=16, border_width=1,
                                      border_color="black")
        self.graph_box.grid(row=0, column=0, sticky="nsew", padx=(0, 20))
        self.graph_box.pack_propagate(False)

        ctk.CTkLabel(self.graph_box, text=localization.get("weekly_activity"), font=("Inter", 32),
                     text_color="black").pack(pady=(15, 5), anchor="w", padx=20)
        self.draw_weekly_chart(self.graph_box)

        # Quick Actions Box
        self.actions_box = ctk.CTkFrame(self.middle_frame, fg_color="#D9D9D9", corner_radius=16, border_width=1,
                                        border_color="black")
        ...
        ctk.CTkLabel(self.table_box, text=localization.get("recent_actions"), font=("Inter", 32),
                     text_color="black").pack(anchor="w", padx=20, pady=(10, 5))
    ...
    def create_stat_card(self, parent, title, value, icon_filename, col):
        card = ctk.CTkFrame(parent, fg_color="#D9D9D9", height=162, corner_radius=16, border_width=1,
                            border_color="black")
    ...
    def draw_weekly_chart(self, parent):
        ...
        # Плейсхолдер для дней недели, если в локализации нет
        days = localization.get("graph_days", ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"])
        ...
        for i in range(7):
            ...
            canvas.create_text(x0 + 15, 190, text=days[i] if i < len(days) else "", fill="black", font=("Inter", 10))

    # --- AUDIT STREAM (ОБНОВЛЕННАЯ ЛОКАЛИЗАЦИЯ ДЛЯ "НОВЫЙ ЧИТАТЕЛЬ") ---
    def load_recent_transactions(self):
        ...
        for i, h in enumerate(headers):
            ctk.CTkLabel(self.transactions_frame, text=h, font=("Inter", 12, "bold"), text_color="gray").grid(row=0,
                                                                                                              column=i,
                                                                                                              padx=5,
                                                                                                              pady=5,
                                                                                                              sticky="w")
        ...
        ctk.CTkFrame(self.transactions_frame, height=1, fg_color="gray").grid(row=1, column=0, columnspan=5,
                                                                              sticky="ew", pady=(0, 5))
        ...
        for row_idx, row_data in enumerate(transactions, start=2):
            for col_idx, item in enumerate(row_data):
                ...
                # Вот магия перевода: пропускаем ЛЮБОЙ текст через localization.get()
                display_text = localization.get(str(item)) if isinstance(item, str) else str(item)

                ctk.CTkLabel(self.transactions_frame, text=display_text, text_color=color, font=("Inter", 13)).grid(
                    row=row_idx, column=col_idx, padx=5, pady=4, sticky="w")

            ctk.CTkFrame(self.transactions_frame, height=1, fg_color="#A0A0A0").grid(row=row_idx * 2 + 1, column=0,
    ...
    def trigger_data_export(self, choice):
        try:
            ...
            messagebox.showinfo(localization.get("exp_success_title", "Экспорт завершен"),
                                f"{localization.get('exp_success_msg', 'Файл сохранен:').format(filename)}")
        except Exception as e:
            self.export_btn.set(localization.get("export"))
            messagebox.showerror(localization.get("exp_error_title", "Ошибка"), str(e))

    def show_add_book_modal(self):
        if hasattr(self, "modal_frame") and self.modal_frame.winfo_exists():
            return

        self.modal_frame = ctk.CTkFrame(self.right_container, width=797, height=597, fg_color="#E8E8E8",
                                        corner_radius=9, border_width=1, border_color="#B0B0B0")
        ...
        # Перевели кнопку ОТМЕНА
        btn_cancel = ctk.CTkButton(fields_wrap, text=localization.get("cancel", "ОТМЕНА"), font=("Inter", 20, "bold"),
        ...
        # Перевели кнопку ДОБАВИТЬ КНИГУ
        btn_add = ctk.CTkButton(fields_wrap, width=211, height=70, text=localization.get("add_book", "ДОБАВИТЬ КНИГУ"),
